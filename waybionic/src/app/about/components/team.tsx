    ...
    <section className="w-full flex flex-col items-center justify-center bg-gray-100 px-6 py-24">
      ...
      <div className="w-full max-w-7xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
        {teamMembers.map((member, index) => (
          <div
            key={index}
            className="bg-white rounded-2xl shadow-xl p-10 text-center transform transition-transform hover:scale-105"
          >
            <h3 className="text-3xl font-semibold mb-4 text-gray-800">
              {member.role}
            </h3>
            <p className="text-2xl text-gray-600 font-medium">{member.name}</p>
          </div>
        ))}
      </div>
    </section>
